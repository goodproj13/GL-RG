...
class CaptionModel(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.use_seg_feature = opt.use_seg_feature

        self.seg_conv_dim = opt.seg_conv_dim

        self.seg_idx = None
        if self.use_seg_feature == 1 and self.seg_conv_dim != 0:
            if self.use_resnet_feature == 1:
                self.seg_idx = 3
            if self.use_resnet_feature == 0:
                self.seg_idx = 2
            if self.use_c3d_feature == 0:
                self.seg_idx = self.seg_idx - 1
        if self.seg_idx is not None:
            self.feat_dims[self.seg_idx] = 10 * 13 * self.seg_conv_dim
            # self.seg_conv = nn.Conv2d(21, self.seg_conv_dim, (3, 3), stride=(1, 1), padding=(1, 1))
            self.seg_conv = nn.Conv2d(21, self.seg_conv_dim, (1, 1), stride=(1, 1))
    ...
    def forward(self, feats, seq):
        ...
        if self.seg_idx is not None:
            seg_feats = feats[self.seg_idx].data.cpu().numpy().reshape((-1, 21, 10, 13))
            # print(seg_feats.shape)
            seg_feats_original = Variable(torch.from_numpy(seg_feats), requires_grad=True)
            if torch.cuda.is_available():
                seg_feats_original = seg_feats_original.cuda()
            seg_feats_embedded = F.relu(self.seg_conv(seg_feats_original))
            seg_feats_reshaped = seg_feats_embedded.view(-1, 1, 10 * 13 * self.seg_conv_dim)
            # print(seg_feats_reshaped.requires_grad)
            # print(seg_feats_original.requires_grad)
            feats[self.seg_idx] = seg_feats_reshaped
